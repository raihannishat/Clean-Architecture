...
async function login(email, password) {
  const result = await callDispatcher('Login', { email, password });
  return result.token;
}

// Get posts
async function getPosts(token) {
  return await callDispatcher('GetBlogPosts', {}, token);
}

// Create a post
async function createPost(token, postData) {
  return await callDispatcher('CreateBlogPost', postData, token);
}

// Get comments
async function getComments(postId) {
  return await callDispatcher('GetComments', { postId });
}
```
